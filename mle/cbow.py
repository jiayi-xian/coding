...
class cbow:
	...
	def __init__(self, text, window=2, n=2, learning_rate=0.0001):
		...
		# input vector matrix for each word ( n X |V| ) each column is a representation of a word in vocabulary
...
final_vectors = {}
for k, v in list(model.word_index.items()):
	final_vectors[k] = (model.U[v, :] + model.V[:, v])/2

print(final_vectors)

import matplotlib.pylab as plt
for k, v in list(final_vectors.items()):
	plt.text(v[0], v[1], k)
