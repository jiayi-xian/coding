"""
Hi, Farina, My name is Jiayi. I was interviewed by you in the afternoon on Dec 09. I really appreciate the time being interviewed by you three. It was internet problem that had some impact
on the interview. Here I write down my explanation to your concern.

Let me rewrite the formula:
let's say v is an innode of u, then 
P(s -> u) = \sum_{innode of u} P(s -> v) * P(v -> u) # P(v->u) is just one step, always

The concern of "cyclic updating": we add a parameter i (0<=i<=k) to denote the current steps we have taken.
then 
P( s-> t, i) = \sum_{innode of t} P(s -> t, i-1) * P(v -> u, 1)
